    ...
    if(get_field('automatic_progress',$id)) {
        ...
        if (get_field('phases_automatic_progress',$id)) {
			...
			if(PSP_PLUGIN_TYPE == 'lite') { 
				...
				$project_phases = get_post_meta($id,'_pano_phases',true);
				...
				if(!empty($project_phases)) { 
					...
					foreach($project_phases as $phase) { 
						...
						if(empty($phase['percentage_complete'])) { 
							$percentage_complete = 0;
						} else { 
							$percentage_complete = $phase['percentage_complete'];
						}
						
						$phase_completion = $phase_completion + $percentage_complete;
				
					}
				
					$phase_total = $phases * 100;
				
					return floor($phase_completion / $phase_total * 100);
