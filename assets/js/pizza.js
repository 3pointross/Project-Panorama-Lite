...
var Pizza = {
  ...
  init : function (scope, options) {
    ...
    var charts = jQuery('[data-pie-id], [data-line-id], [data-bar-id]', this.scope);

    jQuery.extend(true, this.settings, options);

    if (charts.length > 0) {
      charts.each(function () {
        return self.build(jQuery(this), options);
      });
    } else if (jQuery(this.scope).is('[data-pie-id]') 
      || jQuery(this.scope).is('[data-line-id]') 
      || jQuery(this.scope).is('[data-bar-id]')) {
      this.build(jQuery(this.scope), options);
    }
  ...
  events : function () {
    ...
    jQuery(window).off('.pizza').on('resize.pizza', self.throttle(function () {
      self.init();
    }, 500));

    jQuery(this.scope).off('.pizza');
  ...
  build : function(legend, options) {
    legend.data('settings', jQuery.extend({}, this.settings, options, legend.data('options')));
  ...
  data : function (legend, options) {
    ...
    jQuery('li', legend).each(function () {
  ...
  update_DOM : function (parts) {
    ...
    return jQuery(this.identifier(legend)).html(graph);
  },
  ...
  svg : function (legend, settings) {
    var container = jQuery(this.identifier(legend)),
        svg = jQuery('svg', container),
	...
	// this.set_attr(svg, { viewBox: '0 0 275 275', preserveAspectRatio: 'xMinYMin meet' });
...
jQuery.extend(Pizza, {
  bar: function (legend) {
    var settings = legend.data('settings'),
        ...
        current_offset = 0,
        container = jQuery(this.identifier(legend)),
    ...
    var existing_group = jQuery('g[data-id=bars]', svg);
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var existing_rect = jQuery('rect[data-id=r' + i +']', g);
  ...
  assemble_grid : function (svg, min, max, width, height, settings) {
    ...
    if (existing_group.length > 0) {
      var line_g = jQuery('g[data-id=grid]', svg)[0],
          text_g = jQuery('g[data-id=labels]', svg)[0];
    } else {
    ...
    while (i--) {
      if (existing_group.length > 0) {
        var line = jQuery('line[data-id=l' + i + ']', line_g)[0],
            text = jQuery('text[data-id=t' + i + ']', text_g)[0];
      } else {
  ...
  bar_events : function () {
    ...
    // jQuery(this.scope).on('mouseenter.pizza mouseleave.pizza touchstart.pizza', '[data-bar-id] li', function (e) {
    //   var parent = jQuery(this).parent(),
    //       path = jQuery('#' + parent.attr('data-bar-id') + ' rect[data-id=r' + jQuery(this).index() + ']')[0],
    //       settings = jQuery(this).parent().data('settings'),
    //       new_height = parseInt(path.getAttribute('data-y'), 10) + 15;

    //   if (/start/i.test(e.type)) {
    //     jQuery(path).siblings('rect').each(function () {
    //       if (this.nodeName) {
    //         Snap(path).animate({
...
jQuery.extend(Pizza, {
  line : function (legend) {
    ...
    var existing_group = jQuery('g[data-id=line]', svg);

    if (existing_group.length > 0) {
      var line_g = jQuery('g[data-id=line]', svg)[0],
          circle_g = jQuery('g[data-id=points]', svg)[0],
          polyline = jQuery('path[data-id=path]', line_g)[0];
    } else {
    ...
    for (var i = 0; i < data.length; i++) {
      if (existing_group.length > 0) {
        var circle = jQuery('circle[data-id=c' + i + ']', circle_g)[0];
      } else {
  ...
  assemble_grid_x : function (svg, min, max, width, height, settings) {
    var existing_group = jQuery('g[data-id=gridx]', svg);

    if (existing_group.length > 0) {
      var line_g = existing_group[0],
          text_g = jQuery('g[data-id=labelx]', svg)[0];
    } else {
      var line_g = this.svg_obj('g'),
          text_g = this.svg_obj('g');
  ...
  assemble_grid_y : function (svg, min, max, width, height, settings) {
    var existing_group = jQuery('g[data-id=gridy]', svg);

    if (existing_group.length > 0) {
      var line_g = existing_group[0],
          text_g = jQuery('g[data-id=labely]', svg)[0];
    } else {
      var line_g = this.svg_obj('g'),
          text_g = this.svg_obj('g');
  ...
  line_events : function () {
    jQuery(this.scope).on('mouseenter.pizza mouseleave.pizza touchstart.pizza', '[data-line-id] li', function (e) {
      var parent = jQuery(this).parent(),
          path = jQuery('#' + parent.data('line-id') + ' circle[data-id="c' + jQuery(this).index() + '"]')[0],
          settings = jQuery(this).parent().data('settings');

      if (/start/i.test(e.type)) {
        jQuery(path).siblings('circle').each(function () {
          if (this.nodeName) {
            Snap(path).animate({
      ...
      if (/enter|start/i.test(e.type)) {
        ...
        jQuery(path).trigger('mouseenter')
      } else {
        Snap(path).animate({
          transform: 's1 1 ' + path.getAttribute('cx') + ' ' + path.getAttribute('cy')
        }, settings.animation_speed, mina[settings.animation_type]);
        jQuery(path).trigger('mouseout')
      }
    });
  }
...
jQuery.extend(Pizza, {
  pie : function (legend) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var existing_path = jQuery('path[data-id="s' + i + '"]', svg);
      ...
      var existing_text = jQuery('text[data-id="s' + i + '"]', svg);
      ...
      var existing_group = jQuery('g[data-id=g' + i + ']', svg);
  ...
  pie_events : function () {
    jQuery(this.scope).on('mouseenter.pizza mouseleave.pizza touchstart.pizza', '[data-pie-id] li', function (e) {
      var parent = jQuery(this).parent(),
          path = jQuery('#' + parent.attr('data-pie-id') + ' path[data-id="s' + jQuery(this).index() + '"]')[0],
          text = path.nextSibling,
          settings = jQuery(this).parent().data('settings');

      if (/start/i.test(e.type)) {
        jQuery(path).siblings('path').each(function () {
          if (this.nodeName) {
            Snap(path).animate({
              transform: 's1 1 ' + path.getAttribute('data-cx') + ' ' + path.getAttribute('data-cy')
            }, settings.animation_speed, mina[settings.animation_type]);
            Snap(jQuery(this).next()[0]).animate({
              opacity: 0
            }, settings.animation_speed);
          }
        });
